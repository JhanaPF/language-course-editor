...
router.fetchOneDictionary = (req, res) => {
	const { dictionary_name } = req.body;
	if (!dictionary_name) {
		const errMsg = 'Dictionary name is missing';
		console.log(errMsg);
		return res.status(400).json({error: errMsg});
	}

	const dictionnaryList = getDictionaryList();
	if(!dictionnaryList.includes(dictionary_name)){
		console.log('Dictionary name is invalid');
